...
async def fun_image(event: MessageEvent):
    if str(event.message_chain).startswith('/制图'):
        ...
        try:
            if sign:
                img = custom_pic_add_text(
                    text, img_content, font_color=(255, 255, 255), bg_color=0)
            else:
...
def gossip_pic(input_str, path='statics/fun_img/gossip.jpg',
    ...
    draw.text(((w-total_font_size[0])//2, h-50),
              input_str, font=font, fill=fill_color)
    return img
...
def crazy_fan_pic(input_str, path='statics/fun_img/crazy_fan.jpg',
    ...
    draw.text(((w-total_font_size[0])//2, 98-total_font_size[1]//2),
              input_str, font=font, fill=font_color)
    return img
...
def xibao_pic(input_str, path='statics/fun_img/xibao.png',
    ...
    if total_font_size[0] > w-10:
        raise ValueError("请适当缩减字符串长度！")
    draw = ImageDraw.Draw(img)
    draw.text(((w-total_font_size[0])//2, (h-total_font_size[1])//2),
              input_str, font=font, fill=font_color)
    return img
...
def custom_pic_add_text(input_str, img_content,
    ...
    draw.text(((w-total_font_size[0])//2, h+4),
              input_str, font=font, fill=font_color)
    return new_img
...
def mental_support_pic(img_content, path='statics/fun_img/support.png'):
    ...
    user_img = user_img.resize(
        (815, 815), IMG.ANTIALIAS).rotate(23, expand=True)
