...
#地图相关功能
@Listen.group(15)
async def gaode_map(event):
    keyword1='/经纬'
    map_banned_group=[795472143]
    if str(event.message_chain).startswith(keyword1):
        if event.sender.group.id in map_banned_group:
            await send(event,["当前群未开放经纬度功能",Face(face_id=8)])
            return
    ...
    if str(event.message_chain).startswith(keyword2):
        if event.group.id in map_banned_group:
            await send(event,["当前群未开放天气功能",Face(face_id=8)])
        else:
