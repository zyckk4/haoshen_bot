...
plugin = Listen(
    'shit_article_maker',
    '狗屁不通文章生成器和申论生成器,输入“</屁文|申论> 主题”以生成'
)
...
class Slscq:
    ...
    def replace_vn(self, input_str: str) -> str:
        while input_str.find('vn') != -1:
            vn = '，'.join([self.get_verb() + self.get_noun()
    ...
    def replace_sp(self, input_str: str) -> str:
        while input_str.find('sp') != -1:
            input_str = input_str.replace(
                'sp', self.get_parallel_sentence(), 1)
        return input_str
    ...
    def gen(self, them: str = '年轻人买房', essay_num: int = 500) -> dict:
        ...
        while len(begin) < begin_num:
            begin += self.replace_all(self.get_beginning(), them)
        while len(body) < body_num:
            body += self.replace_all(self.get_body(), them)
        while len(end) < end_num:
            end += self.replace_all(self.get_ending(), them)
...
if __name__ == "__main__":
    if 1:
        ...
        paser.add_argument('-n', '--essay_num', help='least character of article',
                           type=int, default=500, metavar='num')
        paser.add_argument('-d', '--data_source', help='json file of data source',
                           type=str, default='../data.json', metavar='json file')
