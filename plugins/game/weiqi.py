...
class Go(ChessWithImg):
    def __init__(self,col=19,row=19,zuozi=0,players=None,
        ...
        self.players=['player1','player2'] if (players is None or players[0]==players[1]) else players
    ...
    def if_dead(self,deadList,yourChessman,coord):
        ...
        for i in (-1,1):
            if [coord[0]+i,coord[1]] not in deadList and 0<=coord[0]+i<self.col and \
                ...
                midvar=self.if_dead(deadList+[[coord[0]+i,coord[1]]],yourChessman,[coord[0]+i,coord[1]])
                if not midvar:
                    return False
                else:
            ...
            if [coord[0],coord[1]+i] not in deadList and 0<=coord[1]+i<self.row and \
                ...
                midvar=self.if_dead(deadList+[[coord[0],coord[1]+i]],yourChessman,[coord[0],coord[1]+i])
                if not midvar:
                    return False
                else:
                    deadList+=copy.deepcopy(midvar)
